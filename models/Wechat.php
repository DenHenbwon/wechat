...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_get($url){
		...
		Yii::warning('http_get request, url:' . $url . ', result:' . json_encode($sContent, JSON_UNESCAPED_UNICODE));
	...
	private function http_post($url,$param,$post_file=false){
		...
		$sContent = curl_exec($oCurl);
		Yii::warning('httP_post request, url:' . $url . ', params:' . json_encode($param, JSON_UNESCAPED_UNICODE) . ', result:' , json_encode($sContent, JSON_UNESCAPED_UNICODE));
		$aStatus = curl_getinfo($oCurl);
		curl_close($oCurl);
		if(intval($aStatus["http_code"])==200){
			return $sContent;
